...
function loadLineDataFromXLS($filename) {	
	...
	if($lrow) {
		$wpdb->query("DELETE FROM lb1706wp_lasta_termin WHERE `polazak_id` IN (SELECT `id` FROM lb1706wp_lasta_polazak WHERE `linija_id`=" . $lrow->id . ")"); 
	...
	for ($row = 5; $row <= $highestRow; ++$row) {
		...
		if(trim($objWorksheet->getCell("A" . $row)->getValue()) == "")
			break;
		$info_stajaliste = $objWorksheet->getCell("B" . $row)->getValue();
		if($info_stajaliste === NULL){
			$info_stajaliste = '';
		}
		$wpdb->insert(
...
<div class="wrap">
	<h2>Import</h2>
	<form id="lasta_form" method="post" action="admin.php?page=lasta-import" enctype="multipart/form-data">

		<input id="rbdomaci" type="radio" name="type" value="Domaci"checked /><label for="rbdomaci">Domaci</label><br/>
		<input id="rbmedjunarodni" type="radio" name="type" value="Medjunarodni" /><label for="rbmedjunarodni">Medjunarodni</label><br/>
		<input id="rbsezonski" type="radio" name="type" value="Sezonski" /><label for="rbsezonski">Sezonski</label><br/>
		<input id="rbgradski" type="radio" name="type" value="Gradski" /><label for="rbgradski">Gradski</label><br/>
		
    	<input type="file" name="Filedata" id="fileToUpload">
    	<input type="submit" value="Upload Image" name="submit">
		
		<div id="lasta_uploading" style="display: none;">Saljem... (<span id="lasta_queue"></span>)</div>
		<div id="lasta_done" style="display: none;"></div>
		<div id="lasta_msg"></div>		
	</form>
</div>
